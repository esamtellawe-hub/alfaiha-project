...
const About = () => {
  ...
  const { sections, loading, error } = useAbout();
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            <div className="inline-flex items-center gap-2 py-1.5 px-4 rounded-full bg-[#ee2039]/20 border border-[#ee2039]/30 text-[10px] font-bold tracking-[0.2em] uppercase mb-6 text-white">
              ...
              {sections?.hero?.subtitle_en || "The Al Faiha Story"}
            </div>

            <h1 className="text-5xl md:text-7xl font-bold mb-6 text-white">
              {sections?.hero?.title_en ? (
                <>
                  {sections.hero.title_en.split(' ')[0]} {sections.hero.title_en.split(' ')[1]} <span className="text-[#ee2039]">{sections.hero.title_en.split(' ').slice(2).join(' ')}</span>
                </>
              ) : (
            ...
            <p className="text-gray-300 text-xl max-w-2xl mx-auto whitespace-pre-line">
              {sections?.hero?.description_en || "A legacy of innovation, integrity, and excellence in construction chemicals across the MENA region."}
            </p>
          </div>
        </div>
      </section>
            ...
            <div className="bg-white rounded-3xl p-8 md:p-12 shadow-xl border border-gray-100 flex flex-col md:flex-row gap-12 items-start relative overflow-hidden">
                ...
                <div className="w-full md:w-2/3">
                    ...
                    <h2 className="text-3xl font-bold text-slate-900 mb-6">
                        {sections?.founder_message?.title_en || "Message from the Founder"}
                    </h2>
                    
                    <div className="space-y-4 text-black leading-relaxed font-light whitespace-pre-line">
                        {sections?.founder_message?.description_en || (
                            <>
                    ...
                    <div className="mt-8 pt-8 border-t border-gray-100">
                        <h4 className="text-xl font-bold text-slate-900">
                            {sections?.founder_message?.extra_data?.founder_name_en || "Ramzi Kandalaft"}
                        </h4>
                        <p className="text-[#ee2039] font-medium">
                            {sections?.founder_message?.extra_data?.founder_title_en || "Founder & CEO of Al Faiha Group"}
                        </p>
                    </div>
                </div>
            </div>
        ...
        <div className="container mx-auto px-4">
            <div className="text-center mb-16">
                <div className="inline-flex items-center gap-2 py-1.5 px-4 rounded-full bg-[#ee2039] border border-[#ee2039]/20 text-[10px] font-bold tracking-[0.2em] uppercase mb-6 text-white">
                  <Target size={12} />
                  {sections?.vision_values?.subtitle_en || "Our Compass"}
                </div>
                <h2 className="text-4xl md:text-5xl font-bold text-slate-900 mb-6">
                  {sections?.vision_values?.title_en ? (
                    <>
                      {sections.vision_values.title_en.split(' ')[0]} <span className="text-[#ee2039]">{sections.vision_values.title_en.split(' ').slice(1).join(' ')}</span>
                    </>
                  ) : (
                    <>Vision & <span className="text-[#ee2039]">values</span></>
                  )}
                </h2>
                <p className="text-xl text-gray-800 max-w-3xl mx-auto whitespace-pre-line">
                    {sections?.vision_values?.description_en || "To be the MENA region’s trusted partner for world-class construction solutions, delivering end-to-end expertise in construction chemicals that combine innovation, quality, and sustainability."}
                </p>
            </div>
                    ...
                    <div className="relative z-10">
                        <h3 className="text-2xl md:text-3xl font-bold mb-6 flex flex-wrap items-center gap-2 md:gap-3">
                            <span className="text-[#ee2039]">
                                {sections?.vision_values?.extra_data?.alfaiha_label_en?.split(' ')[0] || "A.L.F.A.I.H.A."}
                            </span> 
                            <span>
                                {sections?.vision_values?.extra_data?.alfaiha_label_en?.split(' ').slice(1).join(' ') || "Values"}
                            </span>
                        </h3>
                        <p className="text-gray-300 mb-8 leading-relaxed text-sm md:text-base whitespace-pre-line">
                            {sections?.vision_values?.extra_data?.alfaiha_desc_en || "The name A.L.F.A.I.H.A. reflects the principles that guide our work and our growth. Each value represents a commitment we uphold across our operations, partnerships, and solutions."}
                        </p>
                        <ul className="space-y-4">
                            {(sections?.vision_values?.extra_data?.values || [
                                { l_en: "A", t_en: "Ambition for industry leadership" },
                                { l_en: "L", t_en: "Leadership grounded in integrity" },
                                { l_en: "F", t_en: "Future-ready solutions" },
                                { l_en: "A", t_en: "Accountability in every project" },
                                { l_en: "I", t_en: "Innovation in construction solutions" },
                                { l_en: "H", t_en: "Health, Safety, and Sustainability" },
                                { l_en: "A", t_en: "Agility and client responsiveness" },
                            ]).map((item, idx) => (
                                <li key={idx} className="flex items-start md:items-center gap-3 md:gap-4 bg-white/5 rounded-xl p-3 border border-white/10 hover:bg-white/10 transition-colors">
                                    ...
                                    <span className="text-sm md:text-base font-medium text-gray-300">{item.t_en}</span>
                                </li>
                            ))}
                        </ul>
                    </div>
                ...
                <div className="space-y-6">
                   <div className="bg-gray-50 rounded-2xl p-8 border border-gray-100 hover:border-[#ee2039]/30 transition-all">
                       ...
                       <h4 className="text-xl font-bold text-slate-900 mb-3">
                           {sections?.vision_values?.extra_data?.purpose_title_en || "Our Purpose"}
                       </h4>
                       <p className="text-gray-800 leading-relaxed whitespace-pre-line">
                           {sections?.vision_values?.extra_data?.purpose_desc_en || "To bridge global innovation with the Middle East’s evolving construction needs, ensuring structures that are safer, stronger, and more sustainable."}
                       </p>
                   </div>
                   <div className="bg-gray-50 rounded-2xl p-8 border border-gray-100 hover:border-[#ee2039]/30 transition-all">
                       <CheckCircle2 className="text-[#ee2039] mb-4" size={32} />
                       <h4 className="text-xl font-bold text-slate-900 mb-3">
                           {sections?.vision_values?.extra_data?.commitment_title_en || "Our Commitment"}
                       </h4>
                       <p className="text-gray-800 leading-relaxed whitespace-pre-line">
                           {sections?.vision_values?.extra_data?.commitment_desc_en || "These principles are embedded in everything we do; from product development and technical support to project execution and long-term partnerships."}
                       </p>
                   </div>
                </div>
            </div>
      ...
      <section id="our-story" className="py-24 bg-gray-50 relative overflow-hidden">
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-4xl mx-auto text-center mb-20">
            <div className="inline-flex items-center gap-2 py-1.5 px-4 rounded-full bg-[#ee2039] border border-[#ee2039]/20 text-[10px] font-bold tracking-[0.2em] uppercase mb-6 text-white">
              <TrendingUp size={12} />
              {sections?.our_story?.subtitle_en || "Our Journey"}
            </div>
            <h2 className="text-4xl md:text-5xl font-bold text-slate-900 mb-6">
               {sections?.our_story?.title_en ? (
                <>
                  {sections.our_story.title_en.split(' ').slice(0, -1).join(' ')} <span className="text-[#ee2039]">{sections.our_story.title_en.split(' ').slice(-1).join(' ')}</span>
                </>
              ) : (
            ...
            <p className="text-xl text-gray-800 whitespace-pre-line">
              {sections?.our_story?.description_en || "From a pioneering local enterprise to a trusted regional partner."}
            </p>
          </div>
          ...
          <div className="max-w-5xl mx-auto relative">
            ...
            <div className="space-y-12">
              ...
              {(sections?.our_story?.extra_data?.timeline).map((item, idx) => {
                  const isEven = idx % 2 === 0;
                  return (
                      <div key={idx} className="relative flex flex-col md:flex-row gap-8 items-center">
                        <div className={`absolute left-4 md:left-1/2 w-4 h-4 rounded-full ${idx === 1 ? 'bg-white border-4 border-[#ee2039]' : 'bg-[#ee2039] border-4 border-white'} shadow-md z-10 md:-ml-2 transform -translate-x-1.5 md:translate-x-0 mt-6 md:mt-0`}></div>
                        
                        {!isEven && <div className="hidden md:block md:w-1/2"></div>}
                        
                        <div className={`w-full md:w-1/2 ${isEven ? 'md:pr-16 md:text-right pl-12 md:pl-0' : 'md:pl-12 pl-12'}`}>
                          <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100 hover:shadow-xl hover:border-[#ee2039]/20 transition-all duration-300 relative group">
                            <div className={`absolute top-4 right-4 ${isEven ? 'md:right-auto md:left-4' : ''} text-[#ee2039]/10 font-bold text-6xl -z-10 select-none group-hover:text-[#ee2039]/10 transition-colors`}>
                              {item.year_en}
                            </div>
                            <div className="flex justify-start mb-4 relative z-20">
                              <span className={`inline-block px-3 py-1 ${idx === 0 ? 'bg-[#ee2039]/10 text-[#ee2039]' : idx === 1 ? 'bg-blue-50 text-blue-600' : 'bg-green-50 text-green-600'} text-xs font-bold rounded-full`}>
                                {item.badge_en}
                              </span>
                            </div>
                            <h3 className={`text-2xl text-left font-bold text-slate-900 mb-3 text-left`}>{item.title_en}</h3>
                            <p className={`text-gray-800 leading-relaxed text-sm whitespace-pre-line text-left`}>
                              {item.desc_en}
                            </p>
                          </div>
                        </div>
      ...
      <section className="py-24 bg-white relative">
        <div className="container mx-auto px-4">
             <div className="text-center mb-16">
                <div className="inline-flex items-center gap-2 py-1.5 px-4 rounded-full bg-[#ee2039] border border-[#ee2039]/20 text-[10px] font-bold tracking-[0.2em] uppercase mb-6 text-white">
                  <Globe size={12} />
                  {sections?.our_footprint?.subtitle_en || "Our Footprint"}
                </div>
                <h2 className="text-4xl md:text-5xl font-bold text-slate-900 mb-6">
                  {sections?.our_footprint?.title_en ? (
                    <>
                      {sections.our_footprint.title_en.split(' ').slice(0, -2).join(' ')} <span className="text-[#ee2039]">{sections.our_footprint.title_en.split(' ').slice(-2).join(' ')}</span>
                    </>
                  ) : (
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 md:gap-6 max-w-5xl mx-auto">
                {(sections?.our_footprint?.extra_data?.countries || [
                    { name_en: "Jordan" }, { name_en: "Saudi Arabia" }, { name_en: "Iraq" }, 
                    { name_en: "Algeria" }, { name_en: "Lebanon" }, { name_en: "Libya" }
                ]).map((country, idx) => (
                    <div key={idx} className="bg-gray-50 rounded-2xl p-4 md:p-6 border border-gray-100 flex flex-col md:flex-row items-center gap-3 md:gap-4 hover:shadow-lg hover:border-[#ee2039]/20 transition-all group text-center md:text-left h-full justify-center md:justify-start">
                         ...
                         <span className="font-bold text-slate-900 text-sm md:text-lg leading-tight">{country.name_en}</span>
                    </div>
                ))}
            </div>
        </div>
      ...
      <section id="why-us" className="py-24 bg-slate-900 text-white relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 relative z-10">
            <div className="text-center mb-16">
                <div className="inline-flex items-center gap-2 py-1.5 px-4 rounded-full bg-[#ee2039] border border-[#ee2039]/20 text-[10px] font-bold tracking-[0.2em] uppercase mb-6 text-white">
                  <Award size={12} />
                  {sections?.why_choose_us?.subtitle_en || "Why Choose Al Faiha"}
                </div>
                <h2 className="text-4xl md:text-5xl font-bold mb-6">
                  {sections?.why_choose_us?.title_en || "A Legacy of Excellence"}
                </h2>
            </div>

            <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6 max-w-7xl mx-auto">
                {(sections?.why_choose_us?.extra_data?.features || [
                    { icon: "Award", title_en: "Proven Legacy Since 1987", desc_en: "Over three decades of leadership in construction chemicals and materials." },
                    { icon: "Target", title_en: "Comprehensive Solutions", desc_en: "Full portfolio covering Admixtures, Additives & Specialty Materials." },
                    { icon: "HandshakeIcon", title_en: "Exclusive Partnership", desc_en: "Sole licensee manufacturer of European Concrete Additives (ECA)." },
                    { icon: "ShieldCheck", title_en: "Quality & Environmental", desc_en: "ISO 9001 and ISO 14001 certified for consistent quality and safety." },
                    { icon: "Building", title_en: "Track Record", desc_en: "Trusted partner in major infrastructure projects across the region." },
                    { icon: "Users", title_en: "Customer-Centric", desc_en: "Fast, responsive service with tailored technical solutions." },
                    { icon: "Zap", title_en: "QHSE Commitment", desc_en: "Continuous alignment with evolving regulations and safety standards." },
                    { icon: "Heart", title_en: "Trusted Partner", desc_en: "Combining quality, innovation, and regional insight." },
                ]).map((item, idx) => {
                    // Utility to map string icons to components
                    ...
                    return (
                        <div key={idx} className="bg-white/5 rounded-2xl p-6 border border-white/10 hover:bg-white/10 transition-all hover:-translate-y-1">
                            <div className="text-[#ee2039] mb-4">{React.cloneElement(renderIcon(item.icon), { size: 32 })}</div>
                            <h4 className="font-bold text-lg mb-2">{item.title_en}</h4>
                            <p className="text-sm text-gray-300 leading-relaxed">{item.desc_en}</p>
                        </div>
                    );
                })}
            </div>
