...
function AdminNewsSections() {
  ...
  const load = async () => {
    try {
      const r = await axios.get(`${API}/admin/news-sections`, { headers: getHeaders() });
      setItems(r.data);
  ...
  const save = async (e) => {
    e.preventDefault();
    try {
      await axios.put(`${API}/admin/news-sections/${editId}`, form, { headers: getHeaders() });
  ...
  const renderTab = (lang, l) => {
    ...
    return (
      <TabPane tabId={lang}>
        <Row className="mt-3">
          <Col md="6">
            <FormGroup><label>Page Title (e.g. News & Press)</label>
              <Input name={`title_${l}`} value={form[`title_${l}`]} onChange={handleInputChange} />
            </FormGroup>
          </Col>
          <Col md="6">
            <FormGroup><label>Tag Label (e.g. Media Center)</label>
              <Input name={`subtitle_${l}`} value={form[`subtitle_${l}`]} onChange={handleInputChange} />
            </FormGroup>
          </Col>
          <Col md="12">
            <FormGroup><label>Hero Description Text</label>
  ...
  return (
    <div className="content">
      <Row><Col md="12">
        <Card>
          <CardHeader>
            <CardTitle tag="h4">News & Press — Page Settings</CardTitle>
            <h6 className="text-muted">Edit the hero title, tag, and description shown at the top of the News page</h6>
          </CardHeader>
          {msg && <div className="alert alert-success mx-3">{msg}</div>}
          <CardBody>
            <Table responsive>
              <thead className="text-primary"><tr><th>Section</th><th>Title (EN)</th><th>Actions</th></tr></thead>
              <tbody>
                {items.map(item => (
                  <tr key={item.id}>
      ...
      <Modal isOpen={modal} toggle={() => setModal(!modal)} size="lg">
        <ModalHeader toggle={() => setModal(!modal)}>Edit News Hero Section</ModalHeader>
        <Form onSubmit={save}>
          <ModalBody>
            <Nav tabs>
              {['EN', 'AR', 'FR'].map(lang => (
...
export default AdminNewsSections;
